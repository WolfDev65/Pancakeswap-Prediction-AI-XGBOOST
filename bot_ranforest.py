...
def hyperparameter_tuning(X, y):
    param_distributions = {
        ...
        'min_samples_leaf': Integer(1, 50, 'uniform'),
...
def trainModel():
    ...
    with ThreadPoolExecutor(max_workers=5) as executor:
        # Submit tasks for fetching historical data
        for i in range(0, 5000, chunksize):
            future = executor.submit(get_history, num_previous_rounds=i + chunksize)
            futures.append(future)

        # Wait for all the tasks to complete and process the chunk data
        chunk_data_lists = [future.result() for future in futures]
    ...
    model_directory = 'model_ranforest'
